...
export class InsectCatchService {
  private chosenInsectSource = new BehaviorSubject<Insect | null>(null);
  chosenInsect$ = this.chosenInsectSource.asObservable();
  currentScreen: number = 0;
  timer: boolean = false;


  updateChosenInsect(insect: Insect) {
    this.chosenInsectSource.next(insect);
  }
  public insects = [
    new Insect('Fly',
    ...
    new Insect('Roach',
      'Get ready to meet Ralph the Roach! He\'s the undisputed hide-and-seek champion of the bug world. Don\'t bother tidying Mr.Ralph up',
